  ...
  function(req, res) {
    ...
    if (req.body.id) {
      ...
      (updateProjects.projectTitle = req.body.projectTitle),
        ...
        req.files.projectHeroImage
          ? (updateProjects.projectHeroImage =
              req.files.projectHeroImage[0].path)
          : null,
        req.files.projectImageDesktop
          ? (updateProjects.projectImageDesktop =
              req.files.projectImageDesktop[0].path)
          : null,
        req.files.projectImageMobile
          ? (updateProjects.projectImageMobile =
              req.files.projectImageMobile[0].path)
          : null,
        req.files.projectScreenshot1
          ? (updateProjects.projectScreenshot1 =
              req.files.projectScreenshot1[0].path)
          : null,
        req.files.projectScreenshot2
          ? (updateProjects.projectScreenshot2 =
              req.files.projectScreenshot2[0].path)
          : null,
        req.files.projectScreenshot3
          ? (updateProjects.projectScreenshot3 =
              req.files.projectScreenshot3[0].path)
          : null,
        updateProjects.projectsMobileView
          ? (updateProjects.projectMobileView = 'Portrait')
          : (updateProjects.projectMobileView = req.body.projectMobileView);
      console.log('updating project id: ', req.body.id);
      console.log(req.body.projectMobileView);

      Projects.findByIdAndUpdate(req.body.id, updateProjects, function(
        err,
        skill
      ) {
        if (err) {
      ...
      req.files.projectHeroImage
        ? (projects.projectHeroImage = req.files.projectHeroImage[0].path)
        : null,
        req.files.projectImageDesktop
          ? (projects.projectImageDesktop =
              req.files.projectImageDesktop[0].path)
          : null;
      req.files.projectImageMobile
        ? (projects.projectImageMobile = req.files.projectImageMobile[0].path)
        : null;
      req.files.projectScreenshot1
        ? (projects.projectScreenshot1 = req.files.projectScreenshot1[0].path)
        : null;
      req.files.projectScreenshot2
        ? (projects.projectScreenshot2 = req.files.projectScreenshot2[0].path)
        : null;
      req.files.projectScreenshot3
        ? (projects.projectScreenshot3 = req.files.projectScreenshot3[0].path)
        : null;
      updateProjects.projectsMobileView
        ? (updateProjects.projectMobileView = 'Portrait')
        : (updateProjects.projectMobileView = req.body.projectMobileView);
...
router.post(
  ...
  function(req, res) {
    if (req.body.id) {
      ...
      res.json({
        status: 'fail',
        messsage: 'No ID has been specified to remove the project'
      });
    }
  }
);
